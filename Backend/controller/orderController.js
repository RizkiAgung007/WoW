...
export const createOrder = async (req, res) => {
  ...
  try {
    ...
    res.status(201).json({
      message: "Pesanan berhasil terkirim, silahkan tunggu balasan admin.",
...
export const updateOrderStatus = async (req, res) => {
  ...
  try {
    ...
    if (result.affectedRows === 0) {
      return res.status(500).json({ message: "Order tidak ditemukan." });
    }
    ...
    res.json({ message: "Status order berhasil diperbarui." });
  } catch (err) {
    console.error(err.message);
    res.status(500).send("Server Error");
  }
